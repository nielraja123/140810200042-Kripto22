...
void perkalianMatrix(int a[1000][3] , int a_baris , int a_kolom ,  int b[1000][3] , int b_baris , int b_kolom , int res[1000][3]){
...
void cariInvers(int m[3][3] , int n , int m_inverse[3][3] ){
    ...
    int det = cariDeterminan(m , n); // ini menggunakan fungsi cariDeterminan(matrix , order_of_matrix)
...
string encrypt(string pt, int n){
    ...
    while(pt.length() % n != 0){
        pt += "x" ;  // ini digunakan jika plaintext di module dengan matrik tidak sama dengan 0
    }
